...
int bro_bt_client_fist (const bro_fist_t * input_fist, bro_fist_t * out_fist,
                        int spam_sock) {
...
int bro_bt_client_send_fist (const bro_fist_t * input_fist, int spam_sock) {
	...
	for (int i = 0; i < BUFFER_SIZE ; i++){
    	...
    	buffer.size = sizeof(bro_fist_t);
    	memcpy (buffer.packets, &input_fist[i], ( sizeof(bro_fist_t)));                
...
int bro_bt_client_receive_fist (bro_fist_t * out_fist, int spam_sock) {
    size_t res;

	for (int i = 0; i < BUFFER_SIZE ; i++){
		//printf("Receiving packet %d\n", i);
    	res = recv(spam_sock, &out_fist[i], ( sizeof(bro_fist_t)), 0);
	}
